...
// applyHostname reads the current content of filePath as dd-agent, sets the
// hostname field, writes the result back as dd-agent using a temp file, and
// returns the new content. It does NOT restart the service.
func (d *DatadogLinuxOperation) applyHostname(filePath string, hostname string) (string, error) {
	currentContent, err := d.program.ExecuteWithOutput("sudo", []string{}, "-u", "dd-agent", "cat", filePath)
	if err != nil {
		return "", fmt.Errorf("failed to read datadog config file %s: %w", filePath, err)
	}

	newContent := utils.ApplyHostnameInDatadogConfig(currentContent, hostname)

	tmpFile, err := os.CreateTemp("", "orya-dd-hostname-*.yaml")
	if err != nil {
		return "", fmt.Errorf("failed to create temp config file: %w", err)
	}
	tmpFilePath := tmpFile.Name()
